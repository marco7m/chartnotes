...
export function openDetails(
  ...
  const titleEl = header.createEl("div", {
    cls: "chart-notes-details-title",
  });
  titleEl.textContent =
    `Notas em "${label}" (${notes.length})` +
    (Number.isFinite(value) && value !== 0
      ? ` – valor ${
          Number.isInteger(value) ? value : (value as number).toFixed(2)
        }`
      : "");
...
// rótulo curtinho (ex.: para gráficos)
...
export function isLightColor(raw?: string): boolean {
  if (!raw) return false;
...
export interface RenderContext {
  refresh?: () => void;
  reindexFile?: (path: string) => Promise<void> | void;
}
