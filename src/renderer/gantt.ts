...
export function renderGantt(
  ...
  const normalizeFullName = (raw: unknown): string => {
    if (raw == null) return "";
  ...
  // limpa restos de renders anteriores
  Array.from(
  ...
  const tasks = tasksRaw.map((r) => {
    ...
    const props = (r as any).props as Record<string, unknown> | undefined;
  ...
  // ordenação por grupo, depois por data
  validTasks.sort((a, b) => {
  ...
  for (const t of validTasks) {
    if (t.groupKey !== lastGroup) {
      ...
      totalRows += 1; // linha de grupo
    }
    totalRows += 1; // linha da tarefa
  }
  ...
  const drawMultilineLabel = (
    ...
    const maxChars = Math.max(
      8,
      Math.floor(usableWidth / approxCharWidth)
    );
    ...
    lines.forEach((line, idx) => {
      const tNode = document.createElementNS(
        svg.namespaceURI,
        "text"
      ) as SVGTextElement;
  ...
  // botão flutuante pra alternar label compacta / wide
  ...
  const optTooltipFields = Array.isArray(opts.tooltipFields)
    ? opts.tooltipFields.map((s: unknown) => String(s))
    : null;
  ...
  for (const t of validTasks) {
    ...
    rect.setAttribute(
      "fill",
      colorFor(t.row.series ?? fullName, rowIndex)
    );
    ...
    if (due instanceof Date) {
      ...
      const dueLine = document.createElementNS(
        svg.namespaceURI,
        "line"
      ) as SVGLineElement;
...
class GanttEditModal extends Modal {
  ...
  async onOpen(): Promise<void> {
    ...
    const title =
      this.notePath.replace(/\.md$/i, "").split("/").pop() ??
      this.notePath;
